    ...
    pub fn intercept_tool_call(
        ...
        let (result, needs_confirmation, confirmation_message) = match tool_call.name.as_str() {
            ...
            "Execute" => {
                ...
                (exec_result.tool_result, exec_result.needs_confirmation, exec_result.confirmation_message)
            }
            "SummonNano" => {
        ...
        let params_summary = match tool_call.name.as_str() {
            "Read" => tool_call.arguments.get("path").and_then(|v| v.as_str()).unwrap_or("").to_string(),
            "Write" => tool_call.arguments.get("path").and_then(|v| v.as_str()).unwrap_or("").to_string(),
            "Execute" => tool_call.arguments.get("command").and_then(|v| v.as_str()).unwrap_or("").to_string(),
        ...
        kms.record_step(&tool_call.name, &params_summary, &result_summary, result.success);
